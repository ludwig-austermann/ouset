...
#let __wrap(s, dw, c: 0, insert-and: true) = {
  if calc.odd(c) { // left clip for e.g. &=
      if insert-and {
        h(dw)
        $&$
      }
...
#let overset(s, t, c: 0, insert-and: true) = if c == 0 { $limits(#s)^(#t)$ } else {
  context {
...
/// clip param c ∈ {0,1,2,3} ≜ {no clip, left clip, right clip, both clip}, 
/// param insert-and: inserts a & at clip positions
#let underset(s, b, c: 0, insert-and: true) = if c == 0 { $limits(#s)_(#b)$ } else {
  context {
    let sw = measure(s).width
...
/// clip param c ∈ {0,1,2,3} ≜ {no clip, left clip, right clip, both clip}
/// param insert-and: inserts a & at clip positions
#let overunderset(s, t, b, c: 0, insert-and: true) = if c == 0 { $limits(#s)^(#t)_(#b)$ } else {
  context {
    let sw = measure(s).width
    let tw = measure($script(#t)$).width
    let bw = measure($script(#b)$).width
...
#let ouset(..a-sym-over-under-a, insert-and: true) = {
  ...
  let c = if frontand { 1 } else { 0 } + if backand { 2 } else { 0 }
  let rest-params = params.slice(if frontand {1} else {0}, if backand {pl - 1} else {pl})

  overunderset(
    rest-params.at(0, default: []),
    rest-params.at(1, default: []),
    rest-params.at(2, default: []),
    c: c, insert-and: insert-and)
}